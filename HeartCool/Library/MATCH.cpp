...
double CompareBeats(int *beat1, int *beat2, int *shiftAdj)
	...
	for(shift = -MAX_SHIFT; shift <= MAX_SHIFT; ++shift)
		...
		for(i = FIDMARK-(MATCH_LENGTH>>1), beatDiff = 0;
			...
			tempD = beat2[i+shift] ;
			tempD *= scaleFactor ;
			beatDiff += abs((long)(beat1[i] - meanDiff- tempD)) ; // beat2[i+shift]  ) ;
			}
...
int NewBeatType(int *newBeat )
	...
	if(TypeCount < MAXTYPES)
		...
		detect =AnalyzeBeat(&BeatTemplates[TypeCount][0]) ;
		onset = detect[1];
		offset = detect[2];
		amp = detect[4];
	...
	// that has occurred the fewest number of times.
		...
		detect = AnalyzeBeat(BeatTemplates[mcType]) ;
		onset = detect[1];
		offset = detect[2];
		amp = detect[4];
...
void UpdateBeatType(int matchType,int *newBeat, double mi2,
	 int shiftAdj)
	{
	int i,onset,offset,beatBegin, beatEnd ;
	int amp ;
	int * aveBeat  = new int[100];
	int * detect = new int[14];
	...
	detect = AnalyzeBeat(&BeatTemplates[matchType][0]) ;
